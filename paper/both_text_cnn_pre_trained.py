...
class TextCNN(object):
    ...
    Uses an embedding layer, followed by a resized/expand, convolutional, pooling and softmax layer.
    """
    def __init__(self, batch_size, num_classes, embedding_size, resize_n, w2v, l2_reg_lambda=0.0):
        ...
        with tf.name_scope("resize_channel"):
            resized_images = []
            ...
            pooled_outputs = []
            resized_num_filters = 30
            resized_filter_sizes = [12, 15, 20, 15]
            for filter_size in resized_filter_sizes:
                with tf.name_scope("conv-%s" % filter_size):
                    filter_shape = [filter_size, embedding_size, 1, resized_num_filters]
        ...
        with tf.name_scope("max_pooling_channel"):
            resized_images = []
            for i in range(batch_size):
                with tf.name_scope("embedding-%s" % i):
                    bs = tf.reshape(BS[i], [1, -1])
            ...
            pooled_outputs = []
            max_pooling_num_filters = 100
            max_pooling_filter_sizes = [2, 3, 4, 5]
            for filter_size in max_pooling_filter_sizes:
                with tf.name_scope("conv-maxpool-%s" % filter_size):
                    filter_shape = [filter_size, embedding_size, 1, max_pooling_num_filters]
                    W = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.1), name="W")
                    b = tf.Variable(tf.constant(0.1, shape=[max_pooling_num_filters]), name="b")
                    conv = tf.nn.conv2d(
                        resized_embedded_chars_expanded,
                        W,
                        strides=[1, 1, 1, 1],
            ...
            self.h_pool_flat = tf.reshape(self.h_pool, [-1, max_pooling_num_filters * len(max_pooling_filter_sizes)])

        self.out_concat = tf.concat([self.h_concat, self.h_pool_flat], 1)
        with tf.name_scope("dropout"):
            self.h_drop = tf.nn.dropout(self.out_concat, self.dropout_keep_prob)

        out_len = 0
        for filter_size in resized_filter_sizes:
            out_len += resize_n / filter_size
        out_len = out_len * resized_num_filters
        out_len += max_pooling_num_filters * len(max_pooling_filter_sizes)
